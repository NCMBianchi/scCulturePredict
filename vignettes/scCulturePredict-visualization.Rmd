...
create_mock_evaluation <- function() {
  list(
    ...
    direct_table = as.table(matrix(c(8, 1, 1, 1, 7, 2, 1, 2, 7), nrow = 3,
    ...
    svm_table = as.table(matrix(c(9, 0, 1, 1, 8, 1, 0, 2, 7), nrow = 3,
                       dimnames = list(c("A", "B", "C"), c("A", "B", "C")))),
    threshold_table = as.table(matrix(c(7, 1, 0, 2, 6, 1, 1, 2, 8), nrow = 3,
                             dimnames = list(c("A", "B", "C"), c("A", "B", "C"))))
  )
}
...
customized_plot <- base_plot +
  theme_minimal() +
  labs(title = "Highly Customized UMAP Plot",
