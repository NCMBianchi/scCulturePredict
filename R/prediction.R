...
#' @examples
#' \donttest{
#' predictions <- predict_by_similarity(pathway_matrix, signature_matrix)
#' }
predict_by_similarity <- function(pathway_matrix, signature_matrix, threshold = 0.1, verbose = TRUE) {
  ...
  for (i in 1:nrow(pathway_matrix_norm)) {
...
predict_by_svm <- function(pathway_matrix, seurat_object, verbose = TRUE) {
  ...
  # Train-test split
  train_idx <- sample(1:nrow(combined_data), size = 0.8 * nrow(combined_data))
  ...
  colnames(scaled_test) <- c(colnames(pathway_df)[1:(ncol(scaled_test) - 1)], "Condition")
...
train_cell_type_classifier <- function(seurat_object,
  ...
  top_features <- rownames(feature_importance)[1:min(n_features, nrow(feature_importance))]
