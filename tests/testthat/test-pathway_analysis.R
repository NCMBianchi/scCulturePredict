...
test_that("analyze_pathway_enrichment works correctly", {
  ...
  expect_true(all(c("pathway", "n_genes", "mean_expr", "sd_expr", 
                   "p_value", "adj_p_value") %in% names(results)))
...
test_that("create_pathway_heatmap works correctly", {
  ...
  pathway_results <- list(
    pathway_matrix = matrix(rnorm(30), nrow = 3, ncol = 10,
                           dimnames = list(paste0("pathway", 1:3),
                                         paste0("cell", 1:10)))
  )
...
test_that("analyze_pathway_activity works correctly", {
  ...
  expect_s3_class(results, "data.frame")
  expect_true(all(c("pathway", "condition", "mean_activity", "sd_activity",
                   "p_value", "adj_p_value") %in% names(results)))
